...
var MagicString$1 = /*@__PURE__*/getDefaultExportFromCjs(magicString_cjs);
...
function parse(options) {
    ...
    const cacheKey = hash$1(filename + source + JSON.stringify(compilerParseOptions));
...
function compileTemplate(options) {
    ...
    const preprocessor = preprocessLang && consolidate$1[preprocessLang];
    if (preprocessor) {
...
function actuallyCompile(options) {
    const { source, compiler = _compiler, compilerOptions = {}, transpileOptions = {}, transformAssetUrls, transformAssetUrlsOptions, isProduction = (process.env.NODE_ENV === 'production') || (process.env.NODE_ENV === 'test'), isFunctional = false, optimizeSSR = false, isTS = false, bindings } = options;
